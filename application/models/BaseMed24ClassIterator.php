<?php
/*****************************************************************************
*       BaseMed24ClassIterator.php
*
*       Author:  ClearHealth Inc. (www.clear-health.com)        2009
*       
*       ClearHealth(TM), HealthCloud(TM), WebVista(TM) and their 
*       respective logos, icons, and terms are registered trademarks 
*       of ClearHealth Inc.
*
*       Though this software is open source you MAY NOT use our 
*       trademarks, graphics, logos and icons without explicit permission. 
*       Derivitive works MUST NOT be primarily identified using our 
*       trademarks, though statements such as "Based on ClearHealth(TM) 
*       Technology" or "incoporating ClearHealth(TM) source code" 
*       are permissible.
*
*       This file is licensed under the GPL V3, you can find
*       a copy of that license by visiting:
*       http://www.fsf.org/licensing/licenses/gpl.html
*       
*****************************************************************************/


class BaseMed24ClassIterator extends WebVista_Model_ORMIterator implements Iterator {

	public function __construct($dbSelect = null,$autoLoad = true) {
		$this->_ormClass = 'BaseMed24';
		// autoLoad gives an option to query the entire rows which takes time if more data in the table
		if ($autoLoad) {
			parent::__construct($this->_ormClass,$dbSelect);
		}
	}

	public function setFilters($filters) {
		$db = Zend_Registry::get('dbAdapter');
		$dbName = Zend_Registry::get('config')->database->params->dbname;
		$dbSelect = $db->select(null)
            ->from('basemed24', null)
			->distinct('vaclass')
                        ->columns(array(
					"basemed24.vaclass",
					"basemed24.md5",
				))
			->order("vaclass ASC");
			foreach($filters as $filter => $value) {
				switch ($filter) {
					case 'class':
						$dbSelect->join($dbName.'.drugCodeClass AS drugCodeClass', 'drugCodeClass.code = basemed24.vaclass',null)
                        				->columns(array(
								"basemed24.vaclass",
								"basemed24.md5",
								"if (drugCodeClass.code != '',CONCAT(drugCodeClass.code,' - ',drugCodeClass.textShort),basemed24.vaclass) AS vaclass",
							));
						if (strlen($value) > 1) $value  = $value . "%";
						$dbSelect->orWhere("basemed24.vaclass LIKE ?", $value)
							->orWhere('basemed24.tradename LIKE ?', $value)
							->orWhere('basemed24.fda_drugname LIKE ?', $value)
							->orWhere('basemed24.rxnorm LIKE ?', $value)
							->orWhere("drugCodeClass.code LIKE ?", $value)
							->orWhere("drugCodeClass.textShort LIKE ?", $value)
							->group('vaclass');
						break;
					case 'specific':
						if (strlen($value) > 1) $value  = $value . "%";
                        			$dbSelect->columns(array(
								"CONCAT(basemed24.vaclass,' - ',basemed24.tradename,' ',basemed24.strength,' ',basemed24.unit,' ',basemed24.packsize,' ',basemed24.packtype,' ',basemed24.ndc) AS vaclass",
							))
							->orWhere('basemed24.vaclass LIKE ?', $value)
							->orWhere('basemed24.tradename LIKE ?', $value)
							->orWhere('basemed24.fda_drugname LIKE ?', $value)
							->orWhere('basemed24.rxnorm LIKE ?', $value)
							->group('md5');
						break;
					case 'limit':
						$dbSelect->limit((int)$value);
						break;
					case 'formulary':
						$tablePostfix = preg_replace('/[^a-zA-Z0-9]+/','',ucfirst($value));
                                        	$dbSelect->joinLeft($dbName.'.formulary'.$tablePostfix.' as formulary', 'formulary.fullNDC = basemed24.full_ndc',null);
                        			$dbSelect->columns(array(
							"basemed24.vaclass",
							"if (formulary.vaclass != '',formulary.vaclass,basemed24.vaclass) as vaclass"
						));
						if (isset($filters['class']) && strlen($filters['class']) > 1) {
							$class  = $filters['class'] . "%";
							$dbSelect->orWhere("formulary.vaclass like ?", $class);
						}
						break;
					//default:
					//	$dbSelect->where("false");
				}
			}
		//echo $dbSelect->__toString();exit;
		trigger_error($dbSelect->__toString(),E_USER_NOTICE);
		$this->_dbSelect = $dbSelect;
		$this->_dbStmt = $db->query($this->_dbSelect);
	}

}
